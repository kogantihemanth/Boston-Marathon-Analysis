...
library(ggcorrplot)
loadPkg('caret')        # used for creating different models
library(caret)
...
```{r}
#commented out previously organized variables
#bm_2017<-read.csv('marathon_results_2017.csv')
#bm_2017$Official.Time <- as.character(bm_2017$Official.Time) # convert to charachter, the expected input for lubridate
#bm_2017$Official.Time.Min <- period_to_seconds(hms(bm_2017$Official.Time))/60 # divide by 60 to get
#bm_2017$Bib_int <- as.numeric(as.character(bm_2017$Bib))
...
In a model predicting the fourth quarter rank by bib number, gender, and age, slope/intercept value is 1.66e+03 and is significant (p<.001). All predictors, bib number, gender, and age significatly predicts total users (p<0.001).
...
ggplot(data=bm_2017)+
  ...
  ggtitle("Scatter plot of participants Overall Rank vs Forth Quarter Rank (group by gender)")+xlab('Rank of Forth Quarter')+ylab('Overall Rank')
```
```{r}
ggplot(data=bm_2017)+
...
# <span style="color:navy"> Split PCA Analysis </span>
...
```
Principal Component analysis of the split times suggest that 1st Principal Component i.e PC1 covers 97% of the variance in the data. PC1 and PC2 cover 99% of the variance in the data. We can observe this in the above plot.
...
# <span style="color:navy">Analysis using different models</span>
...
colSums(bm == '' | bm == '-')
```
There are `r nrow(bm)` rows after dropping the columns and removing NA and blank values.
...
bm$X25K <- as.character(bm$X25K)
bm$X25K.Min <- period_to_seconds(hms(bm$X25K))/60
bm$X30K <- as.character(bm$X30K)
bm$X30K.Min <- period_to_seconds(hms(bm$X30K))/60
bm$X35K <- as.character(bm$X35K)
bm$X35K.Min <- period_to_seconds(hms(bm$X35K))/60
bm$X40K <- as.character(bm$X40K)
bm$X40K.Min <- period_to_seconds(hms(bm$X40K))/60
...
bm$PaceHalf.25k <- (bm$X25K.Min - bm$Half.Min)/(25-21.1) # Pace during 21.1km(Half time) and 25km
bm$Pace25k.30k <- (bm$X30K.Min - bm$X25K.Min)/5 # Pace during 25km and 30km
bm$Pace30k.35k <- (bm$X35K.Min - bm$X30K.Min)/5 # Pace during 30km and 35km
bm$Pace35k.40k <- (bm$X40K.Min - bm$X35K.Min)/5 # Pace during 35km and 40km
bm$Pace40k.officialtime <- (bm$Official.Time.Min - bm$X40K.Min)/(42.2-40) # Pace during 40km and 42.2km(Official time)
```
...
```{r Extract_data}
# bm_2017_pace = bm[,c('Age','M.F','Pace0k.5k','Pace5k.10k','Pace10k.15k','Pace15k.20k','Pace20k.Half','PaceHalf.25k','Pace25k.30k','Pace30k.35k','Pace35k.40k','Official.Time.Min')]
...
```
We got an r-square value of `r caret::R2(ridge.pred, y_test_scaled)*100`% using Ridge regression.
...
```{r lasso_regression, include=TRUE}
...
dt.fit <- rpart(Official.Time.Min ~ .,method="anova", data=train1)
rsq.rpart(dt.fit)
plot(dt.fit, uniform=TRUE, main="Decision Tree")
text(dt.fit, use.n=TRUE, all=TRUE, digits=5, cex=.8)
dt.pred <- predict(dt.fit, test[ ,-length(test)])
data.frame(
...
We got an r-square value of `r caret::R2(dt.pred, test$Official.Time.Min)*100`% using Decision Tree.
...
We got an r-square value of `r caret::R2(rf_pred, test$Official.Time.Min)*100`% using Random Forest.
...
summary(gb_model)
```
We got an r-square value of `r caret::R2(predict_gbm, test$Official.Time.Min)*100`% using Gradient Boosting. 
We can see that Pace between 15k and 20k and Pace between 20k and 21.1K are by far the most important variables in our gbm model.
...
We got an r-square value of `r caret::R2(predict_xgboost, test$Official.Time.Min)*100`% using Gradient Boosting. 

## Conclusion
* Out of all the models, xgboost is having the better r-square value of `r caret::R2(predict_xgboost, test$Official.Time.Min)*100`%.
...
We can further analyze the model by looking at specific outputs.  The R-squared value of this model is `r round(summary(fit_lin)$adj.r.squared, 5)`.  The p-value for the distance term is `r round(p.lm, 5)`.  The coefficent for the distance variable is `r  round(coef.lm, 5)`.  The VIF is `r vif_lin`.
...
This is an excellent example that really detailed feature engineering and even a very small p-value does not lead to a good model.  Distance traveled results in a low adjusted R-squared value of `r round(p.lm, 5)`, which is definitely stastically significant.  However, the model has little predictive power because of its low adjusted R-squared value and obviously large error residuals.  